...
async def test_fetcher_single_fetch():
    ...
    with aioresponses() as mocked:
        mocked.get(url, status=200, body="test content", repeat=True)
...
async def test_fetcher_same_domain_rate_limiting():
    ...
    with aioresponses() as mocked:
        for url in urls:
            mocked.get(url, status=200, body="test response", repeat=True)
        async with Fetcher() as fetcher:
            start = time.time()
...
async def test_fetcher_cached_requests():
    ...
    with aioresponses() as mocked:
        mocked.get(urls[0], status=200, body="test response", repeat=True)
        async with Fetcher() as fetcher:
            start = time.time()
            await fetcher.fetch_all(urls)
            elapsed = time.time() - start
...
@pytest.mark.asyncio
async def test_fetcher_custom_config():
    """
...
@pytest.mark.asyncio
async def test_fetcher_context_manager_required():
    """
...
@pytest.mark.asyncio
async def test_fetcher_no_retry_when_disabled():
    """
    ...
    assert elapsed == pytest.approx(0.0, abs=IMMEDIATE_TIMING_TOLERANCE)


@pytest.mark.asyncio
async def test_fetcher_retry_delay_timing():
    """
    ...
    assert elapsed == pytest.approx(2.0, abs=RETRY_TIMING_TOLERANCE)


@pytest.mark.asyncio
async def test_fetcher_different_retry_delays():
    """
    ...
    # Verify timing: 2 retries × 1.0s = ~2.0s
    assert elapsed == pytest.approx(2.0, abs=RETRY_TIMING_TOLERANCE)


@pytest.mark.asyncio
async def test_fetcher_retry_on_http_error():
    """
...
async def test_fetcher_retry_then_success():
    ...
    with aioresponses() as mocked:
        ...
        mocked.get(url, status=500)
        mocked.get(url, status=500)
        mocked.get(url, status=200, body="success after retries")
...
@pytest.mark.asyncio
async def test_fetcher_retry_on_connection_error():
    """
    ...
    with aioresponses() as mocked:
        ...
        mocked.get(url, exception=ClientError("Connection failed"))
        mocked.get(url, exception=ClientError("Connection failed"))
        mocked.get(url, status=200, body="success after connection retry")
...
@pytest.mark.asyncio
async def test_fetcher_retry_exhausted():
    """
